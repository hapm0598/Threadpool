...
##########################################
# PID DETECTION (EXACT PIPELINE FROM OLD SCRIPT)
##########################################
...
if [[ -z "${pid:-}" ]]; then
    echo "[error] Could not determine .NET PID"
    exit 1
fi
...
if [[ -z "${instance:-}" ]]; then
    echo "[error] Cannot read COMPUTERNAME from environ"
    exit 1
fi

if [[ -z "${sas_url:-}" ]]; then
    echo "[error] Cannot read SAS URL from environ"
    exit 1
fi
...
##########################################
# UPLOAD FUNCTION (UPLOAD → CLEANUP)
##########################################
upload_and_cleanup() {
    local file="$1"
...
if [[ "$MODE" == "manual" ]]; then
    ...
    if [[ ! -e "$WORKDIR/dump_taken.lock" ]]; then
        echo "[dump] AUTO MODE: Creating first dump lock"
...
echo "[collector] DONE — all files handled."
exit 0